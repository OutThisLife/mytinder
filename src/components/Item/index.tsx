...
export const Item = ({
  ...
  const onClick = () => window.open(`//tinder.com/app/messages/${id}`, '_blank')

  const inc = (messages ?? []).filter(i => i?.from === person?._id)
  const out = (messages ?? []).filter(i => !inc.includes(i))

  console.log(out.at(-1))

  return (
    <Grid {...{ md, sm, xs }}>
      <Card clickable cover hoverable shadow {...{ onPointerDown: onClick }}>
        <Card.Header
          css={{
            display: 'flex',
            gap: '1em',
          ...
          <Col>
            <Row>
              <Text color="#fff" h1>
                {inc.length} / {out.length}
              </Text>
            </Row>

            <Row>
              <Text color="#fff">
                {!!out.length && dayjs(out.at(-1)?.timestamp).fromNow()}
              </Text>
            </Row>
          </Col>
